...
import bisq.network.p2p.node.netty.p2p.handshake.HandshakeHandler;
...
public final class Node implements AutoCloseable {
    ...
    public Node(NettyTcpTransportService transport, int port) {
        this.transport = transport;
        this.port = port;
    }

    public void start() throws Exception {
        transport.startServer(port, new InboundHandshakeHandler(new HandshakeHandler.Handler() {
            @Override
    ...
    public CompletableFuture<Connection> connect(Address address, Duration timeout) {
        return transport.connect(address, new OutboundHandshakeHandler(new HandshakeHandler.Handler() {
            @Override
            public void onHandshakeCompleted(Channel channel) {
