...
// TODO if we use an ApplicationService for multiple apps we need to add "enable" flags to the different services
//  in the config so that we can customize which are activated.
//  E.g. a seed node does not want to activate the SettingService.
@Slf4j
@Getter
public class DaemonApplicationService extends ApplicationService implements CoreServiceProvider {
    ...
    public DaemonApplicationService(String configFileName, String[] args) {
        ...
        switch (bitcoinWalletSelection) {
            case BITCOIND:
                ...
                break;
            case ELECTRUM:
                walletService = Optional.of(new ElectrumWalletService(ElectrumWalletService.Config.from(bitcoinWalletConfig.getConfig("electrum")), getApplicationConfig().getBaseDir()));
                break;
            case NONE:
            default:
        ...
        networkService = new NetworkService(NetworkServiceConfig.from(getApplicationConfig().getBaseDir(),
                getConfig("network")),
                persistenceService,
        ...
        bondedRolesService = new BondedRolesService(BondedRolesService.Config.from(getConfig("bondedRoles")),
                getApplicationConfig().getVersion(),
        ...
        sendNotificationService = new SendNotificationService(getApplicationConfig().getBaseDir(), settingsService);
        ...
        updaterService = new UpdaterService(getApplicationConfig(), settingsService, bondedRolesService.getReleaseNotificationsService());
        ...
        ApiService.Config apiConfig = ApiService.Config.from(getConfig("api"));
        apiService = new ApiService(apiConfig, this);
    }
